 ...
 */
void fann_compute_MSE_p(struct fann *ann, fann_type * desired_output)
{
    ...
    struct fann_neuron *last_layer_it;
    const struct fann_neuron *last_layer_end = last_layer_begin + ann->num_output;
    const struct fann_neuron *first_neuron = ann->first_layer->first_neuron;
    unsigned int nmse = ann->num_MSE;
    float mse = ann->MSE_value;
    unsigned int num_bit_fail = ann->num_bit_fail;
    ...
    #pragma omp parallel for private(neuron_value,neuron_diff) reduction (+:nmse,mse,num_bit_fail)
    for(last_layer_it = last_layer_begin; last_layer_it < last_layer_end; last_layer_it++)
    {
        neuron_value = last_layer_it->value;
        neuron_diff = desired_output[last_layer_it-last_layer_begin] - neuron_value;
        /* fann_update_mse() */
        {
            float neuron_diff2;

            switch (last_layer_it->activation_function)
            {
                case FANN_LINEAR_PIECE_SYMMETRIC:
                case FANN_THRESHOLD_SYMMETRIC:
                case FANN_SIGMOID_SYMMETRIC:
        ...
        error_it[last_layer_it-last_layer_begin] = fann_activation_derived(last_layer_it->activation_function,
                                            last_layer_it->activation_steepness, neuron_value,
                                            last_layer_it->sum) * neuron_diff;
        nmse++;
    }
    ann->MSE_value = mse;
    ann->num_bit_fail = num_bit_fail;
    ann->num_MSE = nmse;
}
...
*/
void fann_backpropagate_MSE_o(struct fann *ann)
{
    fann_type tmp_error;
    unsigned int i;
...
void fann_backpropagate_MSE(struct fann *ann)
{
	...
	unsigned int i;
	struct fann_layer *layer_it;
	struct fann_neuron *neuron_it, *last_neuron, *neuron_begin;
	struct fann_neuron **connections;
	...
	for(layer_it = last_layer - 1; layer_it > second_layer; --layer_it)
	{
		...
		if(ann->connection_rate >= 1)
		{
            ...
            #pragma omp parallel for private(tmp_error,weights,i)
			for(neuron_it = layer_it->first_neuron; neuron_it < last_neuron; neuron_it++)
			{
				tmp_error = error_begin[neuron_it - first_neuron];
				weights = ann->weights + neuron_it->first_con;
		...
		{
            #pragma omp parallel for private(tmp_error,weights,i,connections)
			for(neuron_it = layer_it->first_neuron; neuron_it < last_neuron; neuron_it++)
			{
				tmp_error = error_begin[neuron_it - first_neuron];
				weights = ann->weights + neuron_it->first_con;
		...
		/* then calculate the actual errors in the previous layer */
		error_prev_layer = error_begin + ((layer_it - 1)->first_neuron - first_neuron);
		neuron_begin = (layer_it - 1)->first_neuron;
		last_neuron = (layer_it - 1)->last_neuron;
        #pragma omp parallel for
		for(neuron_it = neuron_begin; neuron_it < last_neuron; neuron_it++)
		{
			error_prev_layer[neuron_it-neuron_begin] *= fann_activation_derived(neuron_it->activation_function,
				neuron_it->activation_steepness, neuron_it->value, neuron_it->sum);
		}
...
*/
void fann_update_slopes_batch_o(struct fann *ann, struct fann_layer *layer_begin,
							  ...
							  struct fann_layer *layer_end)
{
	...
	for(; layer_begin <= layer_end; layer_begin++)
	{
		...
		if(ann->connection_rate >= 1)
		{
            ...
            #pragma omp parallel for private(tmp_error,neuron_slope,num_connections,i)
			for(neuron_it = layer_begin->first_neuron; neuron_it < last_neuron; neuron_it++)
			{
				tmp_error = error_begin[neuron_it - first_neuron];
				neuron_slope = slope_begin + neuron_it->first_con;
		...
		{
            #pragma omp parallel for private(tmp_error,neuron_slope,num_connections,i,connections)
			for(neuron_it = layer_begin->first_neuron; neuron_it < last_neuron; neuron_it++)
			{
				tmp_error = error_begin[neuron_it - first_neuron];
				neuron_slope = slope_begin + neuron_it->first_con;
...
*/
void fann_update_weights_irpropm_o(struct fann *ann, unsigned int first_weight, unsigned int past_end)
{
    fann_type *train_slopes = ann->train_slopes;
    fann_type *weights = ann->weights;
    fann_type *prev_steps = ann->prev_steps;
...
void fann_update_weights_irpropm(struct fann *ann, unsigned int first_weight, unsigned int past_end)
{
	...
	unsigned int i;
    #pragma omp parallel for private(prev_step,slope,prev_slope,same_sign,next_step)
