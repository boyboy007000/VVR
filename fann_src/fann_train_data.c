 ...
 */
FANN_EXTERNAL struct fann_train_data *FANN_API fann_read_train_from_file_b(const char *configuration_file)
{
	...
	data = fann_read_train_from_fd_b(file, configuration_file);
	fclose(file);
	return data;
}
...
float fann_train_epoch_irpropm(struct fann *ann, struct fann_train_data *data)
{
	...
	{
		fann_run(ann, data->input[i]);
		fann_compute_MSE_p(ann, data->output[i]);
		fann_backpropagate_MSE(ann);
		fann_update_slopes_batch(ann, ann->first_layer + 1, ann->last_layer - 1);
		//printf("done %d\n", i);
	}
...
struct fann_train_data *fann_read_train_from_fd_b(FILE * file, const char *filename)
{
	...
	if (fread(&num_data, 4, 1, file) != 1) err = 1;
	if (fread(&num_input, 4, 1, file) != 1) err = 1;
	if (fread(&num_output, 4, 1, file) != 1) err = 1;
	if(err)
	{
	...
	for(i = 0; i != num_data; i++)
	{
		for(j = 0; j != num_input; j++)
		{
			if(fread(&data->input[i][j], FANNSIZE, 1, file) != 1)
			{
				fann_error(NULL, FANN_E_CANT_READ_TD, filename, line);
				fann_destroy_train(data);
				return NULL;
			}
			if (fann_abs(data->input[i][j]) > fann_abs(max)) max = data->input[i][j];
		}
		line++;

		for(j = 0; j != num_output; j++)
		{
			if(fread(&data->output[i][j], FANNSIZE, 1, file) != 1)
			{
				fann_error(NULL, FANN_E_CANT_READ_TD, filename, line);
				fann_destroy_train(data);
				return NULL;
